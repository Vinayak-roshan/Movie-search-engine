...
def load_models():
    count_vectorizer = joblib.load(r"models\count_vectorizer.joblib")
    tfidf_transformer = joblib.load(r"models\tfidf_transformer.joblib")
...
def main():
    ...
    query = st.text_input('Enter your query (Ex: Adventure, comedy, Sci-fi, Sports, Horror etc...)', '')
    if st.button('🔎 Search'):
        if query:
...
if __name__ == '__main__':
    data = load_data(r"data\eng_subtitles_database.csv")
